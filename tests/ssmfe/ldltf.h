...
int num_neg_D(int n, int ld, double *LDLT, int *ipiv) {
   int nneg = 0;
   for(int i=0; i<n; ) {
      double s = LDLT[i*ld + i];
      if( ipiv[i] < 0 ) {
         double t = LDLT[i*ld + i+1];
         double r = LDLT[(i+1)*ld + i+1];
         if( s*r - t*t < 0.0 ) {
            nneg++;
...
int num_neg_Dz(int n, int ld, double complex *LDLT, int *ipiv) {
   int nneg = 0;
   for(int i=0; i<n; ) {
      double s = LDLT[i*ld + i];
      if( ipiv[i] < 0 ) {
         double t = fabs(LDLT[i*ld + i+1]);
         double r = LDLT[(i+1)*ld + i+1];
         if( s*r - t*t < 0.0 ) {
            nneg++;
         } else if ( s*r - t*t > 0.0 && s + r < 0.0 ){ 
