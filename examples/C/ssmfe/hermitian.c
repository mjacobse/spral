...
void apply_idx(int n, int m, const double complex *x, double complex *y) {
   for(int j=0; j<m; j++) {
      for(int i=0; i<n; i++) {
         int il = (i==0)   ? n-1 : i-1;
         int ir = (i==n-1) ? 0   : i+1;
         y[j*n + i] = _Complex_I * (x[j*n + ir] - x[j*n + il]);
      }
   }
}
...
int main(void) {
   ...
   double *lambda = malloc(nep*sizeof(double));              /* eigenvalues */
   double complex *X = malloc(nep*n*sizeof(double complex)); /* eigenvectors */
