...
int test_core_d(int problem, int largest) {
   ...
   int *ind = malloc(m*sizeof(int));          /* permutation index */
   double *lambda = malloc(n*sizeof(double)); /* eigenvalues storage */
   double *X = malloc(n*n*sizeof(double));    /* eigenvectors storage */
   /* Work arrays */
   double *lmd = malloc(m*sizeof(double));
   double *rr = malloc(3*2*m*2*m*sizeof(double));
   double *W = malloc(8*m*n*sizeof(double));
   ...
   for(int i=0; i<n; i++)
   for(int j=0; j<m; j++)
      W[0*m*n + j*n + i] = spral_random_real(&state, true);
   ...
   while(true) { /* reverse communication loop */
      ...
      case 1:
         apply_laplacian(
            ngrid, ngrid, rci.nx, &W[rci.kx*m*n + rci.jx*n + 0], &W[rci.ky*m*n + rci.jy*n + 0]
            );
         if ( largest != 0 )
            cblas_dscal( n*rci.nx, -1.0, &W[rci.ky*m*n + rci.jy*n + 0], 1 );
         break;
      case 2:
         if ( largest != 0 )
            cblas_dcopy( 
               n*rci.nx, &W[rci.kx*m*n + rci.jx*n + 0], 1, &W[rci.ky*m*n + rci.jy*n + 0], 1 
               );
         else
      ...
      case 5:
         ...
         for(int k=0; k<rci.nx; k++) {
           ...
           cblas_dcopy( n, &W[0*m*n + (rci.jx+k)*n + 0], 1, &X[j*n + 0], 1 );
         }
         ncon += rci.nx;
      ...
      case 22:
         if( ncon > 0 ) {
            cblas_dgemm(
               ...
               1.0, X, n, &W[rci.ky*m*n + rci.jy*n + 0], n, 0.0, U, n
               );
            cblas_dgemm(
               CblasColMajor, CblasNoTrans, CblasNoTrans, n, rci.nx, ncon,
               -1.0, X, n, U, n, 1.0, &W[rci.kx*m*n + rci.jx*n + 0], n
               );
            if ( problem != 0 )
              cblas_dgemm(
                 CblasColMajor, CblasNoTrans, CblasNoTrans, n, rci.nx, ncon,
                 -1.0, X, n, U, n, 1.0, &W[rci.ky*m*n + rci.jy*n + 0], n
                 );
         }
         break;
      case 999:
...
int test_core_z(int problem, int largest) {
   ...
   int *ind = malloc(m*sizeof(int));                       /* permutation index */
   double *lambda = malloc(n*sizeof(double));              /* eigenvalues storage */
   double complex *X = malloc(n*n*sizeof(double complex)); /* eigenvectors storage */
   /* Work arrays */
   double *lmd = malloc(m*sizeof(double));
   double complex *rr = malloc(3*2*m*2*m*sizeof(double complex));
   double complex *W = malloc(8*m*n*sizeof(double complex));
   double complex *U = malloc(m*n*sizeof(double complex));
   ...
   while(true) { /* reverse communication loop */
      ...
      case 1:
         ...
         if ( largest != 0 )
            cblas_zscal( n*rci.nx, &MINUS_ONE, &W[rci.ky*m*n + rci.jy*n + 0], 1 );
         break;
      case 2:
         if ( largest != 0 )
            cblas_zcopy( 
      ...
      case 5:
         ...
         for(int k=0; k<rci.nx; k++) {
           int j = ncon + k;
           lambda[j] = lmd[rci.jx + k];
           cblas_zcopy( n, &W[0*m*n + (rci.jx+k)*n + 0], 1, &X[j*n + 0], 1 );
         }
         ncon += rci.nx;
         if ( ncon >= nep || inform.iteration > 300 )
            goto finished; 
      ...
      case 22:
         if( ncon > 0 ) {
            cblas_zgemm(
               CblasColMajor, CblasTrans, CblasNoTrans, ncon, rci.nx, n,
               &ONE, X, n, &W[rci.ky*m*n + rci.jy*n + 0], n, &ZERO, U, n
               );
            cblas_zgemm(
               CblasColMajor, CblasNoTrans, CblasNoTrans, n, rci.nx, ncon,
               &MINUS_ONE, X, n, U, n, &ONE,
               &W[rci.kx*m*n + rci.jx*n + 0], n
               );
            if ( problem != 0 )
              cblas_zgemm(
                 CblasColMajor, CblasNoTrans, CblasNoTrans, n, rci.nx, ncon,
                 &MINUS_ONE, X, n, U, n, &ONE,
                 &W[rci.ky*m*n + rci.jy*n + 0], n
                 );
         }
         break;
      case 999:
...
int test_expert_d(int problem) {
   ...
   int *ind = malloc(m*sizeof(int));          /* permutation index */
   int *ipiv = malloc(n*sizeof(int));         /* LDLT pivot index */
   double sigma;                              /* shift */
   double *A = malloc(n*n*sizeof(double));    /* matrix */
   double *LDLT = malloc(n*n*sizeof(double)); /* factors */
   double *lambda = malloc(n*sizeof(double)); /* eigenvalues */
   double *X = malloc(n*n*sizeof(double));    /* eigenvectors */
   /* Work arrays */
   double *work = malloc(n*n*sizeof(double)); /* work array for dsytrf */
   double *rr = malloc(3*2*m*2*m*sizeof(double));
   double *W = malloc(8*m*n*sizeof(double));
   double *U = malloc(m*n*sizeof(double));
   double *V = malloc(m*sizeof(double));
   ...
   if ( problem > 1 ) {
     ...
     for(int j=0; j<n; j++)
     for(int i=0; i<n; i++)
        LDLT[j*n + i] = (i==j) ? A[j*n + i] - sigma
                               : A[j*n + i];
     cwrap_dsytrf('L', n, LDLT, n, ipiv, work, n*n);
   ...
   while(true) { /* reverse communication loop */
      ...
      case 9:
         ...
         cwrap_dsytrs(
            'L', n, rci.nx, LDLT, n, ipiv, &W[rci.ky*m*n + rci.jy*n + 0], n
            );
         break;
      case 11:
      case 12:
         ...
         if( ncon > 0 ) {
            ...
            if ( problem == 1 || problem == 3 )
              cblas_dgemm(
               ...
               apply_laplacian(
                  ngrid, ngrid, rci.nx, &W[rci.kx*m*n + rci.jx*n + 0], 
                  &W[rci.ky*m*n + rci.jy*n + 0]
                  );
         }
         break;
      case 999:
...
int test_expert_z(int problem) {
   ...
   double sigma;                                           /* shift */
   double *lambda = malloc(n*sizeof(double));              /* eigenvalues */
   double complex *X = malloc(n*n*sizeof(double complex)); /* eigenvectors */
   /* Work arrays */
   double *B = malloc(n*n*sizeof(double));
   double *LDLT = malloc(n*n*sizeof(double));
   ...
   while(true) { /* reverse communication loop */
      ...
      case 9:
         for(int i=0; i<n; i++)
           for(int j=0; j<rci.nx; j++)
              work[j*n + i] = W[rci.kx*m*n + (rci.jx+j)*n + i];
         cwrap_dsytrs(
            'L', n, rci.nx, LDLT, n, ipiv, work, n
            );
         for(int i=0; i<n; i++)
           for(int j=0; j<rci.nx; j++)
              W[rci.ky*m*n + (rci.jy+j)*n + i] = work[j*n + i];
         break;
      case 11:
      case 12:
      case 13:
      ...
      case 22:
         if( ncon > 0 ) {
            ...
            cblas_zgemm(
               CblasColMajor, CblasNoTrans, CblasNoTrans, n, rci.nx, ncon,
               &NONE, X, n, U, n, &ONE, &W[rci.kx*m*n + rci.jx*n + 0], n
               );
            if ( problem == 1 || problem == 3 )
              cblas_zgemm(
                 CblasColMajor, CblasNoTrans, CblasNoTrans, n, rci.nx, ncon,
                 &NONE, X, n, U, n, &ONE, &W[rci.ky*m*n + rci.jy*n + 0], n
                 );
            else if ( problem == 4 )
               apply_laplacian_z(
                  ngrid, ngrid, rci.nx, &W[rci.kx*m*n + rci.jx*n + 0], 
...
int test_ssmfe_d(int problem) {
   ...
   while(true) { /* reverse communication loop */
      switch ( problem ) {
      case 0:
         spral_ssmfe_standard_double( &rci, nep, 2*nep, lambda, n, X, n,
            &keep, &options, &inform );
         break;
      case 1:
         spral_ssmfe_standard_double( &rci, nep, 2*nep, lambda, n, X, n,
...
int test_ssmfe_z(int problem) {
   ...
   while(true) { /* reverse communication loop */
      ...
      case 3:
         ...
         break;
      case 9:
         zdcopy( n*rci.nx, rci.x, work );
         cwrap_dsytrs( 'L', n, rci.nx, LDLT, n, ipiv, work, n );
         dzcopy( n*rci.nx, work, rci.y );
         break;
      default:
         goto finished;
      }
   ...
   switch ( rci.job ) {
         ...
         if ( rci.i == 0 ) {
            if ( rci.kx != rci.ky || rci.jx > rci.jy ) {
               ...
               for(int j=rci.nx-1; j>=0; j--)
                  cblas_dcopy(
                     n, &W[rci.kx*m*n + (rci.jx+j)*n + 0], 1, &W[rci.ky*m*n + (rci.jy+j)*n + 0], 1
                     );
            }
         } else {
            for(int i=0; i<n; i++) {
               for(int j=0; j<rci.nx; j++)
                  U[j] = W[rci.kx*m*n + ind[j]*n + i];
               for(int j=0; j<rci.nx; j++)
                  W[rci.kx*m*n + j*n + i] = U[j];
               if(rci.ky != rci.kx) {
                  for(int j=0; j<rci.nx; j++)
                    U[j] = W[rci.ky*m*n + ind[j]*n + i];
                  for(int j=0; j<rci.nx; j++)
                    W[rci.ky*m*n + j*n + i] = U[j];
               }
            }
         }
         break;
      case 12:
         for(int i=0; i<rci.nx; i++)
           rr[rci.k*2*m*2*m + (rci.j+i)*2*m + rci.i+i] =
             cblas_ddot(
                n, &W[rci.kx*m*n + (rci.jx+i)*n + 0], 1, &W[rci.ky*m*n + (rci.jy+i)*n + 0], 1
                );
         break;
      case 13:
         for(int i=0; i<rci.nx; i++) {
            if( rci.kx == rci.ky ) {
               double s = cblas_dnrm2(n, &W[rci.kx*m*n + (rci.jx+i)*n + 0], 1);
               if( s > 0 )
                  cblas_dscal(n, 1/s, &W[rci.kx*m*n + (rci.jx+i)*n + 0], 1);
            } else {
               ...
               if ( s > 0 ) {
                  cblas_dscal(n, 1/s, &W[rci.kx*m*n + (rci.jx+i)*n + 0], 1);
                  cblas_dscal(n, 1/s, &W[rci.ky*m*n + (rci.jy+i)*n + 0], 1);
               } else {
                  for(int j=0; j<n; j++)
                     W[rci.ky*m*n + (rci.jy+i)*n + j] = 0.0;
               }
            }
         }
         break;
      case 14:
         for(int i=0; i<rci.nx; i++) {
           double s = -rr[rci.k*2*m*2*m + (rci.j+i)*2*m + rci.i+i];
           cblas_daxpy(
              n, s, &W[rci.kx*m*n + (rci.jx+i)*n + 0], 1, &W[rci.ky*m*n + (rci.jy+i)*n + 0], 1
              );
         }
         break;
      case 15:
         if ( rci.nx > 0 && rci.ny > 0 )
            cblas_dgemm(
               ...
               rci.alpha, &W[rci.kx*m*n + rci.jx*n + 0], n, &W[rci.ky*m*n + rci.jy*n + 0], n,
               rci.beta, &rr[rci.k*2*m*2*m + rci.j*2*m + rci.i], 2*m
               );
         break;
      ...
      case 17:
         if( rci.job == 17 ) {
            cblas_dgemm(
               ...
               1.0, &W[rci.kx*m*n + rci.jx*n + 0], n, &rr[rci.k*2*m*2*m + rci.j*2*m + rci.i], 2*m,
               0.0, &W[rci.ky*m*n + rci.jy*n + 0], n
               );
            cblas_dcopy(
               n*rci.ny, &W[rci.ky*m*n + rci.jy*n + 0], 1, &W[rci.kx*m*n + rci.jx*n + 0], 1
               );
         } else {
            cblas_dgemm(
               CblasColMajor, CblasNoTrans, CblasNoTrans, n, rci.ny, rci.nx,
               rci.alpha, &W[rci.kx*m*n + rci.jx*n + 0], n, &rr[rci.k*2*m*2*m + rci.j*2*m + rci.i],
               2*m, rci.beta, &W[rci.ky*m*n + rci.jy*n + 0], n
               );
         }
         break;
   }
  ...
  ( struct spral_ssmfe_rciz rci, 
   ...
   double complex z;
    ...
    case 12:
       for(int i=0; i<rci.nx; i++) {
          cblas_zdotc_sub(
              n, &W[rci.kx*m*n + (rci.jx+i)*n + 0], 1, &W[rci.ky*m*n + (rci.jy+i)*n + 0], 1, &z
              );
          rr[rci.k*2*m*2*m + (rci.j+i)*2*m + rci.i+i] = z;
       }
       break;
    case 13:
       for(int i=0; i<rci.nx; i++) {
          if( rci.kx == rci.ky ) {
             double s = cblas_dznrm2(n, &W[rci.kx*m*n + (rci.jx+i)*n + 0], 1);
             if( s > 0 ) {
                z = ONE/s;
                cblas_zscal(n, &z, &W[rci.kx*m*n + (rci.jx+i)*n + 0], 1);
             }
          } else {
             cblas_zdotc_sub(
                n, &W[rci.kx*m*n + (rci.jx+i)*n + 0], 1, &W[rci.ky*m*n + (rci.jy+i)*n + 0], 1, &z
             ...
             if ( s > 0 ) {
                z = ONE/s;
                cblas_zscal(n, &z, &W[rci.kx*m*n + (rci.jx+i)*n + 0], 1);
                cblas_zscal(n, &z, &W[rci.ky*m*n + (rci.jy+i)*n + 0], 1);
             } else {
                for(int j=0; j<n; j++)
                   W[rci.ky*m*n + (rci.jy+i)*n + j] = 0.0;
             }
    ...
    case 14:
       for(int i=0; i<rci.nx; i++) {
         ...
         z = s;
         cblas_zaxpy(
            n, &z, &W[rci.kx*m*n + (rci.jx+i)*n + 0], 1, &W[rci.ky*m*n + (rci.jy+i)*n + 0], 1
            );
       }
       break;
    case 15:
    ...
    case 17:
       if( rci.job == 17 ) {
          cblas_zgemm(
             CblasColMajor, CblasNoTrans, CblasNoTrans, n, rci.ny, rci.nx,
             &ONE, &W[rci.kx*m*n + rci.jx*n + 0], n, &rr[rci.k*2*m*2*m + rci.j*2*m + rci.i], 2*m,
             &ZERO, &W[rci.ky*m*n + rci.jy*n + 0], n
             );
          cblas_zcopy(
             n*rci.ny, &W[rci.ky*m*n + rci.jy*n + 0], 1, &W[rci.kx*m*n + rci.jx*n + 0], 1
             );
